...
#endif
    ROS_WARN("waiting for event, image and imu...");

    registerPub(n);

    ros::Subscriber sub_imu = n.subscribe(IMU_TOPIC, 2000, imu_callback, ros::TransportHints().tcpNoDelay());
    ros::Subscriber sub_eventfeature = n.subscribe("/stereo_event_tracker/feature", 2000, stereo_eventfeature_callback);//event node
    ros::Subscriber sub_imagefeature = n.subscribe("/stereo_image_tracker/feature", 2000, stereo_imagefeature_callback);//image node
    ros::Subscriber sub_relo_points = n.subscribe("/pose_graph/match_points", 2000, relocalization_callback);

    std::thread measurement_process{process_event_image_imu};
    ros::spin();
